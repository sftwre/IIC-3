...
import org.archs as archs
from org.utils.cluster.general import config_to_str, get_opt, update_lr, nice
from org.utils.cluster.data import cluster_twohead_create_dataloaders
from org.utils.cluster.cluster_eval import cluster_eval, get_subhead_using_loss
from org.utils.cluster.IID_losses import IID_loss
from org.utils.cluster.render import save_progress
...
config.output_k = config.output_k_B  # for eval org
assert (config.output_k_A >= config.gt_k)
config.eval_mode = "hung"

assert ("MNIST" == config.dataset)
dataset_class = torchvision.datasets.MNIST(config.dataset_root, download=True)
...
def train(render_count=-1):
  ...
  for e_i in range(next_epoch, config.num_epochs):
    ...
    for head_i in range(2):
      ...
      writer.add_scalar(f'IIC-MNIST/{head}_train_loss', avg_loss, e_i)
